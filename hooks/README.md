...
| **Secret scan** | `Bash` | Scans staged files for secrets (API keys, tokens, private keys, connection strings) before `git commit`/`git push`. Blocks if secrets detected | 2 (blocks) |
| **Deploy env check** | `Bash` | Validates required env vars exist before deployment commands. Reads from `.env.required`, `.env.example`, or `package.json` `deployEnv` field | 2 (blocks) |
...
| **Pipeline hint** | `Bash` | Suggests next pipeline step after tests pass, build succeeds, or lint completes (e.g. "Tests passed. Consider running /code-review next.") |
...
| **Lessons loader** | `SessionStart` | Loads `tasks/lessons.md` and `tasks/todo.md` into context on session start |
...
| **Progress tracker** | `Stop` | Updates `tasks/todo.md` with last active timestamp after each response |
